...
static bool messaging_consumer_enqueue_packet(message_consumer_t *consumer, telemetry_ref_t *ref) {
    ...
    if (retval != MSG_OK) {
        ...
        messaging_consumer_receive(consumer, false, true);
        retval = chMBPost(&consumer->impl->mailbox, (intptr_t) ref, TIME_IMMEDIATE);
    }
    if (retval == MSG_OK) {
...
// A copy of the data will be made, so you can freely modify/release the data after this call
messaging_send_return_codes
messaging_producer_send_timestamp(message_producer_t *producer, message_metadata_t flags, const uint8_t *data, uint32_t timestamp) {
    if (producer->impl == NULL) {
        COMPONENT_STATE_UPDATE(avionics_component_messaging, state_error);
...
messaging_receive_return_codes messaging_consumer_receive(message_consumer_t *consumer, bool blocking, bool silent) {
    ...
    intptr_t data_msg;
    msg_t mailbox_ret = chMBFetch(&(consumer->impl->mailbox), (msg_t *) &data_msg,
                                  blocking ? TIME_INFINITE : TIME_IMMEDIATE);
